...
contract WasabiPoolFactory is Ownable, IWasabiPoolFactory {
    ...
    mapping (address => PoolStates) private poolStates;
    ...
    function createPool(
        ...
        poolStates[_poolAddress] = PoolStates.ACTIVE;
    ...
    function issueOption(address _target) external returns (uint256) {
        require(poolStates[msg.sender] == PoolStates.ACTIVE, "Only active pools can issue options");
        return options.newMint(_target);
    }

    /// @inheritdoc IWasabiPoolFactory
    function burnOption(uint256 _optionId) external {
        require(poolStates[msg.sender] != PoolStates.INVALID, "Invalid pools can't burn options");
        options.burn(_optionId);
    }

    /// @inheritdoc IWasabiPoolFactory
    function togglePool(address _poolAddress, PoolStates _poolState) external onlyOwner {
        require(poolStates[_poolAddress] != _poolState, 'Pool already in same state');
        poolStates[_poolAddress] = _poolState;
    }

    /// @inheritdoc IWasabiPoolFactory
    function isValidPool(address _poolAddress) external view returns(bool) {
        return poolStates[_poolAddress] == PoolStates.ACTIVE;
    }
