...
contract SeaportV14Module is BaseExchangeModule {
    ...
    chargeETHFees(fees, params.amount)
  {
    ...
    if (params.revertIfIncomplete) {
      ...
      for (uint256 i; i < length; ) {
        uint256 price = orders[i].price;
        _fillSingleOrder(orders[i].order, criteriaResolvers, params.fillTo, price);
  ...
  function acceptERC1155Offer(
    ...
    if (amountFilled > 0) {
      ...
      for (uint256 i; i < feesLength; ) {
        Fee memory fee = fees[i];
          uint256 a = (fee.amount * amountFilled);
        _sendERC20(
          fee.recipient,
        ...
        uint256 aa = (fee.amount * amountFilled);
        _sendERC20(
          fee.recipient,
          // Only pay fees for the amount that was actually filled
          aa/b,
