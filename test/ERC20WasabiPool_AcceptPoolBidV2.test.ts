...
import { toEth, toBN, makeRequest, metadata, gasOfTxn, assertIncreaseInBalance, advanceTime, expectRevertCustomError, withBid, signPoolAskWithEIP712, signPoolBidWithEIP712, getAllTokenIds } from "./util/TestUtilsV2";
import { PoolAsk, OptionType, ZERO_ADDRESS, PoolBid } from "./util/TestTypesV2";
...
import { ERC20WasabiPoolV2Instance, OptionIssued } from "../types/truffle-contracts/ERC20WasabiPoolV2.js";
...
contract("ERC20WasabiPoolV2: Accept Pool Bid", accounts => {
    ...
    it("Write Option (only owner)", async () => {
        ...
        request = makeRequest(0, pool.address, testNft.address, OptionType.CALL, strike, premium, expiry, 1001, orderExpiry);
        ...
        const callOptionData = await pool.getOptionData(callOptionId);
        assert.equal(
            callOptionData.collection,
            testNft.address, 
            "Option of collection not correct");

        // Write PUT and validate
        request = makeRequest(1, pool.address,testNft.address, OptionType.PUT, strike, premium, expiry, 1001, orderExpiry);
        signature = await signPoolAskWithEIP712(request, pool.address, lpPrivateKey);
