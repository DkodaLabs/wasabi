...
import { OptionExecuted, OptionIssued } from "../types/truffle-contracts/IWasabiPoolV2";
import { PoolAsk, OptionType, ZERO_ADDRESS } from "./util/TestTypesV2";
import { assertIncreaseInBalance, expectRevertCustomError, gasOfTxn, makeRequest, metadata, signPoolAskWithEIP712, toBN, toEth } from "./util/TestUtilsV2";
...
contract("Erc20WasabiPool: PutOption", accounts => {
    ...
    it("Validate option requests", async () => {
        ...
        request = makeRequest(id, pool.address, testNft.address, OptionType.PUT, 10, premium, expiry, 1001, orderExpiry); // no premium in request
        ...
        request = makeRequest(id, pool.address, testNft.address, OptionType.PUT, 0, premium, expiry, 1001, orderExpiry); // no strike price in request
        signature = await signPoolAskWithEIP712(request, pool.address, lpPrivateKey);
        ...
        request = makeRequest(id, pool.address, testNft.address, OptionType.PUT, 0, premium, expiry, 0, orderExpiry); // no strike price in request
        signature = await signPoolAskWithEIP712(request, pool.address, lpPrivateKey)
        await expectRevertCustomError(
            pool.writeOption.sendTransaction(request, signature, metadata(buyer)),
            "InvalidStrike");
        
        request = makeRequest(id, pool.address, testNft.address, OptionType.PUT, strikePrice, 0, expiry, 0, orderExpiry); // no premium in request
        signature = await signPoolAskWithEIP712(request, pool.address, lpPrivateKey)
        await truffleAssert.reverts(
            pool.writeOption.sendTransaction(request, signature, metadata(buyer)),
            "Not enough premium is supplied",
        ...
        request = makeRequest(id, pool.address, testNft.address, OptionType.PUT, initialPoolBalance * 5, premium, expiry, 0, orderExpiry); // strike price too high
        signature = await signPoolAskWithEIP712(request, pool.address, lpPrivateKey)
        await expectRevertCustomError(
            pool.writeOption.sendTransaction(request, signature, metadata(buyer)),
        ...
        request = makeRequest(id, pool.address, testNft.address, OptionType.PUT, strikePrice, premium * 2, expiry, 0, orderExpiry);
        ...
        request = makeRequest(id, pool.address, testNft.address, OptionType.PUT, strikePrice, premium, expiry, 0, orderExpiry);

        signature = await signPoolAskWithEIP712(request, pool.address, buyerPrivateKey)
        await expectRevertCustomError(
        ...
        const request2 = makeRequest(id, pool.address, testNft.address, OptionType.PUT, strikePrice, 0.1, expiry, 0, orderExpiry);
