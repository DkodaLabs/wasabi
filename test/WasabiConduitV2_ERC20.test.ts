...
contract("WasabiConduitV2 ERC20", (accounts) => {
  ...
  it("Execute Option (only option holder)", async () => {
    ...
    await token.approve(
      pool.address,
      toEth((strikePrice * (feeDenominator + feeValue)) / feeDenominator),
      metadata(buyer)
    );
  ...
  it("Issue Option", async () => {
    ...
    await token.approve(
      pool.address,
      toEth((premium * (feeDenominator + feeValue)) / feeDenominator),
      metadata(buyer)
    );
  ...
  it("Accept ask", async () => {
    ...
    const initialRoyaltyReceiverBalance = await token.balanceOf(
      royaltyReceiver
    );
    ...
    assert.equal(
      fromWei(initialBalanceBuyer.sub(finalBalanceBuyer)),
      price,
      "Buyer incorrect balance change"
    );
    ...
    // Fee Manager
    assert.equal(
      fromWei(finalRoyaltyReceiverBalance.sub(initialRoyaltyReceiverBalance)),
      royaltyAmount,
  ...
  it("Accept bid", async () => {
    ...
    const initialBalanceSeller = await token.balanceOf(optionOwner);
    const acceptBidResult = await conduit.acceptBid(
      optionId,
      pool.address,
      bid,
      signature,
  ...
  it("PoolAcceptBid with invalid pool address", async () => {
    ...
    await truffleAssert.reverts(
      conduit.poolAcceptBid(bid, signature, 0, metadata(lp)),
      "Pool is not valid"
    );
  });
  ...
  it("Cancel bid", async () => {
    ...
    const cancelBidResult = await conduit.cancelBid(
      bid,
      signature,
      metadata(someoneElse)
    );
