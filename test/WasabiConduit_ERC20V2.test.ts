...
import { toEth, toBN, makeRequest, metadata, signPoolAskWithEIP712, fromWei, signBidWithEIP712, signAskWithEIP712, expectRevertCustomError, getAllTokenIds } from "./util/TestUtilsV2";
import { Ask, Bid, OptionData, PoolAsk, OptionType, ZERO_ADDRESS } from "./util/TestTypesV2";
...
contract("WasabiConduitV2 ERC20", accounts => {
    ...
    it("Issue Option", async () => {
        ...
        let expiry = timestamp + 10000;
        let orderExpiry = timestamp + 10000;
        request = makeRequest(request.id, pool.address, testNft.address, OptionType.CALL, 10, 1, expiry, 1002, orderExpiry);
